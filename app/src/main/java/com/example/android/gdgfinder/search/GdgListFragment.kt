...
class GdgListFragment : Fragment() {
    ...
    override fun onCreateView(
        ...
        viewModel.regionList.observe(viewLifecycleOwner) { regionList ->
            regionList?.let {
                ...
                val children: List<Chip> = it.map { regionName ->
                    val chip = inflater.inflate(R.layout.region, chipGroup, false) as Chip
                    chip.text = regionName.ifEmpty { "Other" }
                    chip.tag = regionName
                    chip.id = chipIdByRegion(regionName)
                ...
                viewModel.gdgList.value?.let { gdgList ->
                    chipGroup.check(chipIdByRegion(gdgList[0].region))
                }
            }
        }
    ...
    private fun chipIdByRegion(region: String): Int {
        return when (region) {
            "Africa" -> 1
            "Asia" -> 2
            "Europe" -> 3
